...
const shortcutIncomeSchema = z.object({
  ...
  description: z.string().nullish().transform((v) => v?.trim() || 'Shortcut Income'),
  ...
  idempotencyKey: z.string().nullish().transform((v) => v?.trim() || `sh_inc_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`),
});
